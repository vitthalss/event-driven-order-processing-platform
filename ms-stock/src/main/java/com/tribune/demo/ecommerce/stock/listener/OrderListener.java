...
/**
 * Stock Order Listener - listens to order events from Kafka
 */
@Slf4j
@Component
public record OrderListener(OrderService orderService) {
    ...
    public void onEvent(Order o) {
        try {
            ...
            log.info("Received order event: ID={}, Status={}, ProductId={}",
                    o.getId(), o.getStatus(), o.getProductId());
            ...
            if (OrderStatus.NEW.equals(o.getStatus())) {
                log.debug("Processing new order for stock: {}", o.getId());
