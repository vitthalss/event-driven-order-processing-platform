...
public class TestKafkaConfig {
    ...
    public KafkaListenerContainerFactory<ConcurrentMessageListenerContainer<Long, Order>> kafkaListenerContainerFactory() {
        return new ConcurrentKafkaListenerContainerFactory<Long, Order>() {
            @Override
            public ConcurrentMessageListenerContainer<Long, Order> createListenerContainer(KafkaListenerEndpoint endpoint) {
    ...
    public KafkaStreamsConfiguration kStreamsConfig() {
        return new KafkaStreamsConfiguration(java.util.Map.ofEntries(
            java.util.Map.entry(StreamsConfig.APPLICATION_ID_CONFIG, "test-streams-app"),
            java.util.Map.entry(StreamsConfig.BOOTSTRAP_SERVERS_CONFIG, "localhost:9092")
        ));
    }
}
